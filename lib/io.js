...
DatumWriter.prototype = {
    ...
    writeData: function(writersSchema, datum, encoder, jsonPath) {
        if (!jsonPath) {
            jsonPath = []
        }
        ...
        writersSchema.validateAndThrow(writersSchema.type, datum, jsonPath);
        ...
        switch(writersSchema.type) {
            ...
            case "array":   this.writeArray(writersSchema, datum, encoder, jsonPath); break;
            case "map":     this.writeMap(writersSchema, datum, encoder, jsonPath); break;
            case "union":   this.writeUnion(writersSchema, datum, encoder, jsonPath); break;
            case "record":
            case "errors":
            case "request": this.writeRecord(writersSchema, datum, encoder, jsonPath); break;
            default:
                throw new AvroErrors.IOError("Unknown type: path [%j], schema %j for data %j", jsonPath, writersSchema, datum);
        }
    },
    ...
    writeArray: function(writersSchema, datum, encoder, jsonPath) {
        if (!jsonPath) {jsonPath = []}
        var self = this;
        if (datum.length > 0) {
            encoder.writeLong(datum.length);
            _.each(datum, function(item) {
                var fPath = jsonPath.slice(0);
                fPath.push(item)

                self.writeData(writersSchema.items, item, encoder, fPath);
            });
        }
        encoder.writeLong(0);
    },

    writeMap: function(writersSchema, datum, encoder, jsonPath) {
        if (!jsonPath) {jsonPath = []}
        var self = this;
        if (_.size(datum) > 0) {
            ...
            _.each(datum, function(value, key) {
                ...
                self.writeData(writersSchema.values, value, encoder,fPath);
            })
        }
        encoder.writeLong(0);
    },

    writeUnion: function(writersSchema, datum, encoder, jsonPath) {
        if (!jsonPath) {jsonPath = []}
        var schemaIndex = -1;
        ...
        if (schemaIndex < 0) {
            throw new AvroErrors.IOError("No schema found for field, [%j], data %j", jsonPath, datum);
        } else {
            encoder.writeLong(schemaIndex);
            this.writeData(writersSchema.schemas[schemaIndex], datum, encoder, jsonPath);
        }
    },

    writeRecord: function(writersSchema, datum, encoder, jsonPath) {
        if (!jsonPath) {jsonPath = []}
        var self = this;
        _.each(writersSchema.fields, function(field) {
            try {
                var fPath = jsonPath.slice(0);
                fPath.push(field.name)
                self.writeData(field.type, datum[field.name], encoder, fPath);
            } catch (err) {
                if (err.fieldPath) {
