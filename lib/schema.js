        ...
        switch (schema) {
            ...
            case 'record':
                ...
                if (intersect.length < dFields.length)
                    throw new AvroErrors.DataValidationError("Data in path [%j], has extra fields not in schema: [%j]. \n Data [%j] \n data fields [%j]. \n schema fields [%j]", path, datum, _.difference(dFields, intersect),dFields, fields);
                break;
            default:
                break;
        }
